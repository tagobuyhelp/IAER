...
import {
  ...
  Banknote, ClipboardCheck, School, Hotel, Quote, CircleDollarSign, Target, Video, GraduationCap, MapPin, BarChart, Settings, MonitorPlay, ArrowRight, Menu, X, Layers, Search, Terminal, Cloud, Clock, Database, PieChart, Calculator
} from 'lucide-react';
import Link from 'next/link';
import Marquee from "react-fast-marquee";

export default function DataScienceLandingPage() {
  ...
  useEffect(() => {
    ...
    console.log("[Meritto Data Science] Hook mounted. Target widget ID:", btnId);

    // Ensure hidden button exists
    let btn = document.querySelector('.npfWidget-' + btnId);
    if (!btn) {
      console.log("[Meritto Data Science] Creating hidden button element");
  ...
  const onApplyNow = () => {
    const btnId = '387bb313db2bcf961b9ec24aba3d71af';
    const baseUrl = 'widgets.nopaperforms.com';
    console.log("[Meritto Data Science] onApplyNow function invoked");
    try {
      const widget = window['npfW' + btnId];
      console.log("[Meritto Data Science] Checked widget instance on window:", widget);
      if (widget && typeof widget.showPopup === 'function') {
        console.log("[Meritto Data Science] Invoking widget.showPopup directly");
        widget.showPopup(btnId, baseUrl);
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-300 scroll-smooth font-sans pb-20 sm:pb-0">
      ...
      <div className="relative w-full border-b border-violet-500/20 bg-slate-950 text-[11px] text-violet-100 shadow-sm sm:text-sm">
        <div className="pointer-events-none absolute inset-0 opacity-20" aria-hidden="true">
          <div className="absolute -left-10 top-0 h-20 w-40 rounded-full bg-violet-500 blur-2xl" />
        </div>
        <div className="relative mx-auto flex max-w-[1550px] items-center px-4 py-1.5 overflow-hidden">
          <div className="flex flex-1 overflow-hidden font-bold text-violet-400 tracking-wide">
            <Marquee speed={30} autoFill pauseOnHover>
              ...
              <div className="flex items-center uppercase tracking-widest text-[9px] sm:text-[10px] text-violet-100">
                MAKAUT Affiliated
              </div>
              <div className="mx-6 md:mx-12">✦</div>
              <div className="flex items-center uppercase tracking-widest text-[9px] sm:text-[10px] text-fuchsia-400">
                AICTE Approved
              </div>
              <div className="mx-6 md:mx-12 text-primary">✦</div>
              <div className="flex items-center uppercase tracking-widest text-[9px] sm:text-[10px] text-violet-100">
              ...
              <div className="mx-6 md:mx-12">✦</div>
              <div className="flex items-center uppercase tracking-widest text-[9px] sm:text-[10px]">
                Eastern India's Best Data Science College
              </div>
            </Marquee>
          </div>
        </div>
      ...
      {/* Header */}
      <header className="sticky top-0 z-40 border-b border-violet-500/20 bg-slate-950/80 backdrop-blur-md shadow-[0_4px_30px_rgba(16,185,129,0.05)]">
        <div className="mx-auto flex max-w-[1550px] items-center justify-between px-4 py-3">
          ...
          <nav className="hidden items-center gap-6 lg:gap-8 text-base text-slate-300 lg:flex">
            {[
              ...
              <a key={l.href} href={l.href} className="group relative flex items-center gap-1.5 rounded-md px-1.5 py-1 transition-colors duration-200 hover:text-violet-400">
                <l.icon className="h-5 w-5" />
                <span className="font-semibold">{l.label}</span>
                <span className="pointer-events-none absolute -bottom-1 left-1.5 right-1.5 h-[2px] origin-left scale-x-0 rounded-full bg-violet-500 transition-transform duration-200 group-hover:scale-x-100" />
              </a>
            ))}
            <a href="tel:+916292004641" className="ml-2 inline-flex items-center gap-2 rounded-full border border-violet-500/30 bg-violet-500/10 px-4 py-1.5 text-sm font-bold text-violet-100 hover:bg-violet-500/20 transition-colors">
              <Phone className="h-4 w-4 text-violet-400" />
              <span>6292004641</span>
            </a>
            <Button onClick={onApplyNow} className="ml-3 rounded-full bg-violet-500 px-6 py-2.5 text-base font-bold text-slate-950 shadow-[0_0_15px_rgba(16,185,129,0.4)] ring-1 ring-violet-400/50 transition-all hover:-translate-y-0.5 hover:bg-violet-400 hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] flex items-center">
              Enquire Now <ArrowRight className="ml-1.5 h-5 w-5" />
            </Button>
          </nav>

          <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="rounded-md p-2 text-violet-400 transition-colors hover:bg-violet-500/10 lg:hidden" aria-label="Toggle menu">
            {mobileMenuOpen ? <X className="h-6 w-6" /> : <Menu className="h-6 w-6" />}
          </button>
        </div>

        {/* Mobile menu */}
        <div className={`overflow-hidden border-t border-violet-500/20 bg-slate-950 shadow-xl transition-all duration-300 ease-in-out lg:hidden ${mobileMenuOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
          <nav className="flex flex-col space-y-1 px-4 py-4 text-sm text-slate-300">
            {[
              ...
              <a key={l.href} href={l.href} onClick={() => setMobileMenuOpen(false)} className="flex items-center gap-2 rounded-md px-3 py-2.5 transition-colors hover:bg-violet-500/10 hover:text-violet-400">
      ...
      {/* --- HERO --- */}
      <section className="relative overflow-hidden bg-slate-950 pt-6 sm:pt-8 pb-8 sm:pb-12 border-b border-violet-500/20">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <img src="/images/programs/bsc-in-data-science-banner.webp" alt="Data Science SOC" className="h-full w-full object-cover opacity-40 mix-blend-luminosity" />
        ...
        <div className="pointer-events-none absolute inset-0 z-0 flex justify-center opacity-50">
          <div className="absolute top-[-10%] left-[-10%] h-[600px] w-[600px] rounded-full bg-violet-500/10 blur-[120px]" />
          <div className="absolute bottom-[-10%] right-[-10%] h-[500px] w-[500px] rounded-full bg-fuchsia-500/10 blur-[100px]" />
        </div>
        ...
        <div className="relative z-10">
          <div data-animate-on-scroll className="relative z-10 mx-auto grid max-w-[1550px] grid-cols-1 gap-4 sm:gap-6 px-4 py-4 md:grid-cols-12 md:gap-6 md:py-6 transition-all duration-700 ease-out">
            <div className="flex-1 md:col-span-7 space-y-3 sm:space-y-4">
              <div className="font-mono text-[10px] sm:text-xs font-bold text-violet-400 uppercase tracking-widest mb-2">
                B.Sc. DATA SCIENCE | MACHINE LEARNING | ARTIFICIAL INTELLIGENCE
              </div>
              <div className="inline-flex items-center gap-1.5 sm:gap-2 rounded-full bg-violet-500/10 backdrop-blur-sm border border-violet-500/30 px-2.5 py-1 sm:px-3 sm:py-1.5 text-[11px] sm:text-xs font-medium text-violet-100 shadow-[0_0_15px_rgba(139,92,246,0.15)]">
                <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-violet-400 rounded-full animate-pulse shadow-[0_0_5px_#a78bfa]" />
                <span>Best Data Science College in Kolkata with Placement</span>
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <div className="flex items-center gap-1 sm:gap-1.5 text-[10px] sm:text-xs font-semibold text-white mt-1 sm:mt-2" aria-label="Student rating">
                  {[...Array(5)].map((_, i) => (
                    <Star key={i} className="h-3 w-3 sm:h-5 sm:w-5 text-violet-400 drop-shadow-[0_0_5px_rgba(16,185,129,0.5)]" fill="currentColor" />
                  ))}
              ...
              <div className="flex items-center w-full sm:w-auto gap-3 mt-4">
                <Button onClick={onApplyNow} size="lg" className="flex-1 sm:flex-none h-9 sm:h-12 rounded-full bg-violet-500 px-4 sm:px-10 text-xs sm:text-sm font-bold text-slate-950 shadow-[0_0_15px_rgba(16,185,129,0.4)] transition-all hover:-translate-y-0.5 hover:bg-violet-400 hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] flex justify-center items-center">
                  Enquire Now <ArrowRight className="ml-1 sm:ml-2 h-3.5 w-3.5 sm:h-5 sm:w-5" />
                </Button>
                <Button onClick={onDownload} variant="outline" size="lg" className="flex-1 sm:flex-none h-9 sm:h-12 rounded-full border border-violet-500/50 text-violet-400 bg-violet-500/5 hover:bg-violet-500/20 px-4 sm:px-10 text-xs sm:text-sm font-bold transition-all flex justify-center items-center backdrop-blur-sm">
              ...
              <div className="mt-5 sm:mt-6 flex items-center gap-2 sm:gap-4">
                <div className="flex items-center gap-1.5 sm:gap-3 bg-slate-900/50 backdrop-blur-sm border border-violet-500/20 rounded-xl sm:rounded-2xl px-2.5 sm:px-3.5 py-1.5 sm:py-2 hover:bg-slate-900 transition-all duration-200 shadow-sm">
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <span className="text-[10px] sm:text-xs font-black text-violet-100 leading-tight mt-0.5">MAKAUT, WB</span>
                  </div>
                </div>
                <div className="flex items-center gap-1.5 sm:gap-3 bg-slate-900/50 backdrop-blur-sm border border-violet-500/20 rounded-xl sm:rounded-2xl px-2.5 sm:px-3.5 py-1.5 sm:py-2 hover:bg-slate-900 transition-all duration-200 shadow-sm">
                  ...
                  <div className="flex flex-col text-left">
                    ...
                    <span className="text-[10px] sm:text-xs font-black text-violet-100 leading-tight mt-0.5">AICTE, Govt</span>
                  </div>
                </div>
              </div>
            ...
            <div id="enquiry-form-section" className="flex flex-col md:col-span-5 items-center justify-center gap-4 sm:gap-5 mt-4 md:mt-0 w-full relative z-20">
              {/* Form Wrapper */}
              <div className="relative w-full max-w-[400px] mx-auto bg-slate-900/80 backdrop-blur-xl rounded-2xl shadow-[0_0_40px_rgba(16,185,129,0.15)] border border-violet-500/30 p-2 sm:p-3 min-h-[420px] overflow-hidden ring-1 ring-inset ring-violet-500/10">
                <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-violet-500 to-transparent opacity-50" />
              ...
              <div className="w-full mt-2 lg:-ml-4">
                <p className="font-mono text-[10px] font-bold text-violet-500/80 uppercase tracking-widest text-center md:text-left mb-3">
                    ...
                    {[...Array(2)].map((_, i) => (
                      <React.Fragment key={i}>
                        {[
                          ...
                          <div key={`${i}-${idx}`} className="flex items-center gap-2.5 bg-slate-900/90 backdrop-blur-sm rounded-xl p-2 pr-4 shadow-[0_4px_15px_rgba(0,0,0,0.4)] border border-violet-500/20 min-w-max transition-transform hover:-translate-y-0.5 hover:border-violet-500/40">
                            ...
                            <div className="flex flex-col">
                              <span className="text-[11px] font-extrabold text-violet-100 whitespace-nowrap">{student.name}</span>
      ...
      <section id="about" className="relative bg-slate-950 py-6 sm:py-10 border-b border-violet-500/20">
        <div className="mx-auto max-w-[1550px] px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
            <div className="bg-slate-900 border border-violet-500/20 rounded-2xl p-4 sm:p-5 flex flex-col justify-center shadow-[0_0_15px_rgba(16,185,129,0.05)] hover:border-violet-500/50 transition-colors group">
              ...
              <p className="text-sm sm:text-base font-semibold text-violet-100">4 Years</p>
            ...
            <div className="bg-slate-900 border border-violet-500/20 rounded-2xl p-4 sm:p-5 flex flex-col justify-center shadow-[0_0_15px_rgba(16,185,129,0.05)] hover:border-violet-500/50 transition-colors group">
              ...
              <p className="text-sm sm:text-base font-semibold text-violet-100">10+2 (Any Stream)</p>
            ...
            <div className="bg-slate-900 border border-violet-500/20 rounded-2xl p-4 sm:p-5 flex flex-col justify-center shadow-[0_0_15px_rgba(16,185,129,0.05)] hover:border-violet-500/50 transition-colors group">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-violet-500/10 rounded-lg group-hover:bg-violet-500/20 transition-colors">
                  <Banknote className="w-4 h-4 sm:w-5 sm:h-5 text-violet-400" />
                </div>
                <h3 className="text-[10px] sm:text-[11px] uppercase tracking-widest font-bold text-slate-400">Fees & Scholarship</h3>
              </div>
              <p className="text-sm sm:text-base font-semibold text-violet-100">₹4,12,000</p>
            ...
            <div className="bg-slate-900 border border-violet-500/20 rounded-2xl p-4 sm:p-5 flex flex-col justify-center shadow-[0_0_15px_rgba(16,185,129,0.05)] hover:border-violet-500/50 transition-colors group">
              <div className="flex items-center gap-3 mb-2">
                <div className="p-2 bg-violet-500/10 rounded-lg group-hover:bg-violet-500/20 transition-colors">
                  <MapPin className="w-4 h-4 sm:w-5 sm:h-5 text-violet-400" />
                </div>
                <h3 className="text-[10px] sm:text-[11px] uppercase tracking-widest font-bold text-slate-400">Learning Mode</h3>
              </div>
              <p className="text-sm sm:text-base font-semibold text-violet-100">On-Campus</p>
      ...
      <section id="placements" className="relative border-b border-violet-500/20 bg-slate-900 overflow-hidden py-8 sm:py-12">
        ...
        <div data-animate-on-scroll className="relative z-10 mx-auto max-w-[1200px] px-4 flex flex-col items-center">
          ...
          <div className="flex flex-col md:flex-row w-full gap-6 md:gap-10 items-center justify-between mb-8">
            <div className="text-center md:text-left flex-shrink-0">
              ...
              <h2 className="text-2xl sm:text-3xl font-extrabold text-violet-400 tracking-tight">
                Placement <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-fuchsia-500">Highlights</span>
              </h2>
            </div>

            <div className="flex flex-wrap sm:flex-nowrap gap-3 sm:gap-4 w-full md:w-auto">
              <div className="flex-1 bg-gradient-to-br from-violet-500 to-[#0b1c3a] p-4 rounded-2xl shadow-md text-white text-center flex flex-col justify-center min-w-[120px] hover:-translate-y-1 transition-transform">
              ...
              <div className="flex-1 bg-slate-900 border border-violet-500/20 p-4 rounded-2xl shadow-sm text-center flex flex-col justify-center min-w-[100px] hover:-translate-y-1 transition-transform">
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-0.5">Average</div>
                <div className="text-2xl sm:text-3xl font-extrabold text-violet-400">₹4.5+</div>
              </div>
              <div className="flex-1 bg-slate-900 border border-violet-500/20 p-4 rounded-2xl shadow-sm text-center flex flex-col justify-center min-w-[100px] hover:-translate-y-1 transition-transform">
                <div className="text-[10px] text-slate-400 uppercase tracking-wider font-bold mb-0.5">Offers</div>
                <div className="text-2xl sm:text-3xl font-extrabold text-violet-400">350+</div>
              </div>
            </div>
          </div>
                  ...
                  {[...Array(2)].map((_, i) => (
                    <React.Fragment key={`row1-${i}`}>
                      {[
                        ...
                        <div key={idx} className="flex items-center justify-center bg-slate-900 rounded-xl border border-violet-500/20 shadow-sm w-[130px] h-[55px] sm:w-[160px] sm:h-[70px] hover:shadow-[0_0_15px_rgba(16,185,129,0.15)] hover:border-violet-500/50 transition-all group">
          ...
          <div className="flex flex-col md:flex-row w-full justify-between items-center gap-5">
            <div className="flex flex-wrap justify-center md:justify-start gap-2 max-w-2xl">
              {[
                ...
                { name: 'Data Scientist', icon: Search },
                { name: 'ML Engineer', icon: Settings },
                { name: 'BI Analyst', icon: BarChart },
                { name: 'Python Developer', icon: Terminal }
              ].map((role) => (
                <span key={role.name} className="inline-flex items-center gap-1.5 bg-slate-900 text-slate-400 rounded-full px-3 py-1.5 text-[10px] sm:text-xs font-semibold border border-violet-500/20 shadow-sm cursor-default hover:text-violet-400 hover:border-violet-500/50 hover:bg-slate-800 transition-all">
                  <role.icon className="w-3.5 h-3.5 text-violet-400" /> {role.name}
                </span>
              ))}
            </div>
            
            <button onClick={() => { document.getElementById('enquiry-form-section')?.scrollIntoView({ behavior: 'smooth' }); }} className="bg-violet-500 hover:bg-violet-400 text-slate-950 font-bold py-3 px-8 rounded-full shadow-[0_0_15px_rgba(16,185,129,0.4)] hover:shadow-[0_0_25px_rgba(16,185,129,0.6)] hover:-translate-y-0.5 transition-all flex items-center gap-2 text-sm whitespace-nowrap">
            ...
            {/* --- SPECIALIZATIONS --- */}
      <section id="specializations" className="relative border-b border-violet-500/20 bg-slate-900">
        <div data-animate-on-scroll className="mx-auto max-w-[1200px] px-4 py-6 sm:py-8">
          <div className="flex flex-col gap-1 mb-6 text-center sm:text-left">
            ...
            <h2 className="text-2xl sm:text-4xl font-extrabold text-violet-400 tracking-tight">
              Data Science <span className="text-transparent bg-clip-text bg-gradient-to-r from-primary to-fuchsia-500">Curriculum</span>
            </h2>
            <p className="text-sm sm:text-base font-medium text-slate-400 mt-2">Master the core pillars of data analytics, machine learning, and artificial intelligence.</p>
            <div className="mx-auto sm:mx-0 mt-4 h-1.5 w-24 rounded-full bg-violet-500 shadow-sm" />
          </div>
            ...
            {[
              {
                ...
                desc: 'Master Python programming, Data Structures, Algorithms, and Object-Oriented paradigms for data analysis.',
                icon: Terminal,
              ...
              },
              {
                ...
                desc: 'Learn Data Cleaning, Statistical Analysis, and Visualizations using modern Business Intelligence tools.',
                icon: BarChart,
              ...
              },
              {
                ...
                desc: 'Building Predictive Models, Supervised & Unsupervised Learning, and Real-world ML Applications.',
                icon: Database,
              ...
              },
              {
                ...
                desc: 'Hadoop, Spark, Data Engineering principles, and managing large-scale industry datasets.',
                icon: Cloud,
              ...
              <div key={spec.id} className="group relative bg-slate-900 border border-violet-500/20 rounded-2xl p-5 hover:shadow-xl transition-all duration-300 hover:-translate-y-1 overflow-hidden flex flex-col h-full">
                ...
                <h3 className="text-lg font-bold text-violet-400 mb-2 leading-tight">{spec.title}</h3>
 ...
 {/* --- VIDEO TESTIMONIALS --- */}
      <section className="py-10 sm:py-16 bg-violet-500/10 relative overflow-hidden">
        ...
        <div className="mx-auto max-w-[1550px] px-4 relative z-10">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white mb-3">
              Hear It From Our <span className="text-fuchsia-400">Students</span>
            </h2>
            ...
            <div className="mt-4 h-1.5 w-16 bg-violet-500 mx-auto rounded-full" />
          </div>
          ...
          <div className="flex gap-4 sm:gap-6 md:justify-center overflow-x-auto pb-6 pt-2 snap-x snap-mandatory no-scrollbar">
            {[
              ...
              <div 
                key={idx} 
                className="group relative flex-shrink-0 w-[160px] sm:w-[200px] md:w-[240px] aspect-[9/16] rounded-2xl overflow-hidden shadow-xl border border-violet-500/10 bg-slate-900 snap-center sm:snap-start"
              >
                <iframe
      ...
      <section id="why-iaer" className="relative border-b border-violet-500/20 bg-slate-950 py-10 sm:py-16 overflow-hidden">
        {/* Background Pattern */}
        <div className="absolute inset-0 z-0 bg-[linear-gradient(to_right,#80808012_1px,transparent_1px),linear-gradient(to_bottom,#80808012_1px,transparent_1px)] bg-[size:24px_24px]" />
        
        <div className="relative z-10 mx-auto max-w-[1550px] px-4">
          <div className="text-center mb-10 sm:mb-14">
            <span className="text-[10px] sm:text-[11px] font-bold uppercase tracking-widest text-violet-400 mb-2 block">Excellence in Education</span>
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-violet-400 tracking-tight">
              Why Choose <span className="text-white">IAER</span> for Data Science?
            </h2>
            <div className="mt-4 mx-auto h-1.5 w-16 rounded-full bg-violet-500" />
          </div>
          ...
          <div className="grid lg:grid-cols-12 gap-8 lg:gap-12 items-start mb-12">
            ...
            <div className="lg:col-span-5 flex flex-col gap-5 sm:gap-6">
              <div className="relative h-[250px] sm:h-[320px] lg:h-[280px] overflow-hidden rounded-3xl shadow-2xl ring-1 ring-slate-200 group">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-violet-500/90 via-[#143674]/20 to-transparent" />
                <div className="absolute bottom-0 left-0 right-0 p-6">
                  <h3 className="text-xl sm:text-2xl font-bold text-white mb-2 flex items-center gap-2"><BookOpen className="w-6 h-6 text-fuchsia-400" /> About B.Sc. Data Science at IAER</h3>
                  <p className="text-sm text-blue-100 leading-relaxed">
                    We provide a perfect blend of theoretical knowledge and hands-on data analytics training, preparing students for real-world artificial intelligence and machine learning challenges.
                  </p>
                </div>
              </div>

              {/* Badges */}
              <div className="bg-slate-900 rounded-2xl p-5 shadow-sm border border-violet-500/20">
                <h4 className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-4">Core Pillars</h4>
                <div className="flex flex-wrap gap-2">
                  {[
                    { label: 'Practical machine learning training' },
                    { label: 'Live dataset analysis' },
                    { label: 'Internship-driven curriculum' },
                    { label: 'Strong placement support' }
                  ].map((i) => (
                    <span key={i.label} className="inline-flex items-center bg-slate-950 border border-violet-500/20 text-violet-400 px-3 py-1.5 rounded-full text-[11px] sm:text-xs font-semibold cursor-default hover:border-violet-500/50 hover:bg-slate-800 transition-all">
                      <CheckCircle className="w-3.5 h-3.5 mr-1.5 text-fuchsia-400" /> {i.label}
                    </span>
                  ))}
                </div>
              </div>
            ...
            <div className="lg:col-span-7 grid sm:grid-cols-2 gap-4 sm:gap-5 lg:gap-6 h-full">
              {[
                ...
                { icon: Building2, title: 'Industry Integration', desc: 'Real-world predictive modeling, case studies, & tech connections.' },
                { icon: Globe, title: 'Global Exposure', desc: 'International immersion programs in Thailand, Malaysia, Vietnam, & UAE.' },
                { icon: Award, title: 'Skill Development', desc: 'Focused training on data engineering, machine learning, and problem-solving.' },
                { icon: Lightbulb, title: 'Tech Incubation', desc: 'Mentorship & incubation support for future tech entrepreneurs.' },
              ].map((item, idx) => (
                <div key={item.title} className={`bg-slate-900 border border-violet-500/20 p-5 rounded-2xl hover:shadow-[0_0_15px_rgba(16,185,129,0.15)] hover:border-violet-500/50 transition-all duration-300 group flex flex-col justify-center ${idx === 4 ? 'sm:col-span-2' : ''}`}>
                  <div className="flex items-start gap-4">
                    <div className="w-12 h-12 flex-shrink-0 rounded-xl bg-slate-950 border border-violet-500/20 flex items-center justify-center group-hover:border-violet-500/50 group-hover:bg-slate-800 group-hover:scale-110 transition-all duration-300 shadow-sm">
                      <item.icon className="w-6 h-6 text-violet-400 group-hover:text-violet-300 transition-colors" />
                    </div>
                    <div>
                      <h4 className="text-base sm:text-lg font-bold text-violet-400 mb-1">{item.title}</h4>
          ...
          <div className="bg-violet-500/10 rounded-3xl p-6 sm:p-8 shadow-xl relative overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-6 relative z-10">
              <div className="flex-shrink-0 text-center sm:text-left sm:pr-6 sm:border-r sm:border-white/20">
                <h3 className="text-lg sm:text-xl font-bold text-white flex items-center justify-center sm:justify-start gap-2 mb-1"><Trophy className="w-5 h-5 text-fuchsia-400" /> Awards &</h3>
                <p className="text-sm font-medium text-fuchsia-400">Recognized Excellence</p>
              </div>
              
              <div className="flex-1 w-full overflow-hidden relative">
                {/* Fade edges */}
                <div className="absolute top-0 bottom-0 left-0 w-8 bg-gradient-to-r from-violet-500 to-transparent z-10 pointer-events-none" />
                <div className="absolute top-0 bottom-0 right-0 w-8 bg-gradient-to-l from-violet-500 to-transparent z-10 pointer-events-none" />
                ...
                <div className="flex gap-4 sm:gap-6 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory no-scrollbar">
                  {[
                    ...
                    <div key={i} className="flex-shrink-0 snap-start bg-slate-900/10 backdrop-blur-sm border border-violet-500/10 rounded-xl px-4 py-3 min-w-[200px] max-w-[220px]">
      ...
      {/* --- TESTIMONIALS --- */}
      <section className="relative py-8 sm:py-10 bg-slate-900 border-b border-violet-500/20">
        <div className="mx-auto max-w-[1550px] px-4">
          <div className="text-center mb-6 sm:mb-8">
            <h2 className="text-2xl sm:text-3xl font-bold text-violet-400 mb-2">Voices of Success</h2>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="min-w-0">
              <h3 className="text-lg font-semibold text-violet-400 flex items-center gap-2 mb-4"><GraduationCap className="w-5 h-5 text-primary" /> Student Testimonials</h3>
              <div className="flex gap-4 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory no-scrollbar">
                {[
                  { name: 'Proshanto Kar', company: 'Wipro', quote: 'The Data Science program gave me real tech exposure and helped me get placed.' },
                  { name: 'Sneha Gupta', company: 'TCS', quote: 'The B.Sc. Data Science program helped me choose the right career path with a Placement.' }
                ].map((t, i) => (
                  <div key={i} className="group relative flex-shrink-0 w-[290px] sm:w-[340px] snap-start bg-slate-900 border border-violet-500/20 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
            ...
            <div className="min-w-0">
              <h3 className="text-lg font-semibold text-violet-400 flex items-center gap-2 mb-4"><Briefcase className="w-5 h-5 text-primary" /> Industry Testimonials</h3>
              <div className="flex gap-4 overflow-x-auto pb-4 pt-1 snap-x snap-mandatory no-scrollbar">
                {[
                  { name: 'Avijit Basu', company: 'NHRD', quote: 'IAER students are industry-ready and well-trained in data science fundamentals.' },
                  ...
                  <div key={i} className="group relative flex-shrink-0 w-[290px] sm:w-[340px] snap-start bg-slate-900 border border-violet-500/20 rounded-2xl p-5 shadow-sm hover:shadow-md transition-shadow overflow-hidden">
                    <Quote className="absolute top-4 right-4 w-12 h-12 text-violet-500/10 group-hover:text-violet-500/30 transition-colors" />
                    <p className="text-sm text-slate-300 italic mb-5 relative z-10 leading-relaxed h-16 line-clamp-3">"{t.quote}"</p>
                    <div className="flex items-center gap-3 relative z-10">
                      <img src="/images/logos/IAER_ICON.jpg" alt={t.name} className="w-10 h-10 rounded-full object-cover ring-2 ring-violet-500/20 group-hover:ring-violet-500/50 transition-all" />
                      <div>
                        <div className="text-sm font-bold text-white">{t.name}</div>
                        <div className="text-[11px] font-semibold uppercase tracking-wider text-fuchsia-400">{t.company}</div>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      <section id="campus-facilities" className="py-8 sm:py-10 bg-slate-950 border-b border-violet-500/20">
        <div className="mx-auto max-w-[1550px] px-4">
          <div className="grid lg:grid-cols-2 gap-6 sm:gap-8">

            <div>
              <h2 className="text-2xl font-bold text-violet-400 mb-4 flex items-center gap-2"><Video className="w-6 h-6 text-fuchsia-400" /> Campus & Learning Experience</h2>
              <p className="text-sm text-slate-400 mb-4">Watch How IAER Builds Future Data Science Experts</p>
            ...
            <div className="space-y-6 sm:space-y-8">
              <div className="bg-slate-900 rounded-3xl p-5 sm:p-6 shadow-sm ring-1 ring-slate-200">
                <h2 className="text-lg font-bold text-violet-400 mb-1 flex items-center gap-2"><Hotel className="w-5 h-5 text-primary" /> Hostel & Campus Facilities</h2>
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <span key={i} className="group inline-flex items-center gap-1.5 bg-slate-950 border border-violet-500/20 hover:border-primary/50 rounded-full px-3 py-1.5 text-[11px] sm:text-xs font-semibold text-slate-700 hover:bg-blue-50/50 transition-colors cursor-default">
              ...
              <div className="bg-slate-900 rounded-3xl p-5 sm:p-6 shadow-[0_0_15px_rgba(16,185,129,0.1)] ring-1 ring-violet-500/20 relative overflow-hidden group">
                <div className="absolute top-0 right-0 w-32 h-32 bg-violet-500/10 rounded-full blur-3xl -mr-10 -mt-10 group-hover:scale-150 transition-transform duration-700" />
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4 relative z-10">
                  <h2 className="text-lg font-bold text-violet-400 flex items-center gap-2"><Banknote className="w-5 h-5 text-primary" /> Scholarships & Loans</h2>
                ...
                <div className="flex flex-wrap gap-2 relative z-10">
                  {[
                    ...
                    <span key={i} className="inline-flex items-center gap-1.5 bg-slate-950 border border-violet-500/20 shadow-sm rounded-lg px-3 py-2 text-[11px] sm:text-xs font-bold text-violet-400 hover:shadow-md hover:border-violet-500/50 transition-all">
            ...
            {/* --- FAQ --- */}
      <section className="py-10 sm:py-16 bg-slate-950 border-b border-violet-500/20">
        <div className="mx-auto max-w-[800px] px-4">
          <div className="text-center mb-10 sm:mb-12">
            <h2 className="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-violet-400 mb-3">Frequently Asked Questions</h2>
            <p className="text-slate-400 text-sm sm:text-base">Everything you need to know about the Data Science program.</p>
            <div className="mt-4 h-1.5 w-16 bg-violet-500 mx-auto rounded-full" />
          </div>
          <div className="flex flex-col gap-3">
            {[
              { q: 'Which is the best Data Science college in Kolkata?', a: 'IAER is among the leading Data Science colleges offering Ethical Hacking, Digital Forensics, and Cyber Defense training.' },
              { q: 'Is Data Science a good career after 12th?', a: 'Yes, Data Science is one of the fastest-growing and highest-paying technology careers globally.' },
              { q: 'What skills will I learn in B.Sc. Data Science?', a: 'Students learn Ethical Hacking, Network Security, Digital Forensics, Python, Cloud Security, and Cyber Defense techniques.' },
              ...
              { q: 'What is the average salary after B.Sc. Data Science?', a: 'The average package is ₹4.50+ LPA, with higher opportunities based on skills and certifications.' },
              { q: 'What are the career opportunities after B.Sc. Data Science?', a: 'Students can pursue careers as Data Science Analysts, Ethical Hackers, SOC Analysts, Security Consultants, and Digital Forensics Specialists.' },
              ...
              <div key={i} className="bg-slate-900 border border-violet-500/20 rounded-2xl shadow-sm overflow-hidden transition-all duration-300">
                <button 
                ...
                >
                  <span className={`font-bold text-sm sm:text-base pr-4 ${openFaq === i ? 'text-primary' : 'text-violet-400'}`}>{faq.q}</span>
      ...
      {/* --- FOOTER CTA --- */}
      <footer className="bg-violet-500/10 border-t border-violet-500/10 pt-8 sm:pt-10 pb-6">
        <div className="mx-auto max-w-[1550px] px-4 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4 mb-8 sm:mb-10">
            <Button onClick={onApplyNow} size="lg" className="w-full sm:w-auto rounded-full bg-violet-500 hover:bg-violet-500/90 text-white font-bold px-6 sm:px-8 py-5 text-base shadow-lg flex justify-center items-center">
          ...
          <div className="border-t border-violet-500/10 pt-6 flex flex-col md:flex-row justify-between items-center gap-4 text-xs sm:text-sm text-blue-200/60">
      ...
      <div className="fixed bottom-0 md:bottom-6 left-0 right-0 md:left-1/2 md:right-auto md:-translate-x-1/2 z-[60] w-full md:w-max bg-slate-900/95 backdrop-blur-xl border-t md:border border-violet-500/20/80 md:rounded-full shadow-[0_-10px_40px_rgba(0,0,0,0.08)] p-1.5 md:p-2 flex items-center justify-between gap-2 transition-all duration-300">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 flex-shrink-0 pr-1">
          <Button onClick={onApplyNow} className="h-9 sm:h-10 bg-violet-500 hover:bg-violet-500/90 text-white font-extrabold rounded-full px-3 sm:px-5 text-[11px] sm:text-xs shadow-lg shadow-accent/20 active:scale-95 transition-all whitespace-nowrap flex items-center justify-center gap-1.5">
            <GraduationCap className="w-3.5 h-3.5 hidden sm:block" /> Enquire Now
          </Button>
          <a href="tel:+916292004641" className="h-9 sm:h-10 bg-violet-500/10 hover:bg-violet-500/10/90 text-white font-extrabold rounded-full px-3 sm:px-5 text-[11px] sm:text-xs shadow-lg shadow-[#143674]/20 active:scale-95 transition-all flex items-center justify-center gap-1.5 whitespace-nowrap">
