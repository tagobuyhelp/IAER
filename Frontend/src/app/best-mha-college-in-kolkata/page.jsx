...
export default function MHALandingPage() {
  ...
  const heroImages = [
    ...
    '/images/programs/master-of-hospital-administration-careers.webp',
    '/images/programs/master-of-hospital-administration-curriculum.webp'
  ];
  ...
  useEffect(() => {
    ...
    console.log("[Meritto MHA] Hook mounted. Target widget ID:", btnId);

    // Ensure hidden button exists
    let btn = document.querySelector('.npfWidget-' + btnId);
    if (!btn) {
      console.log("[Meritto MHA] Creating hidden button element");
      ...
      console.log("[Meritto MHA] Hidden button element already exists in DOM");
    }
    ...
    const initWidget = () => {
      ...
      console.log("[Meritto MHA] initWidget run. Constructor found:", !!NpfConstructor);
      if (NpfConstructor) {
        if (!window['npfW' + btnId]) {
          console.log("[Meritto MHA] Initializing new NpfWidgetsInit for", btnId);
          ...
          console.log("[Meritto MHA] NpfWidgetsInit instance created:", window['npfW' + btnId]);
        } else {
          console.log("[Meritto MHA] NpfWidgetsInit instance already exists");
        }

        const trigger = document.querySelector('.npfWidget-' + btnId);
        if (trigger) {
          console.log("[Meritto MHA] Binding onclick to trigger button");
          trigger.onclick = (e) => {
            console.log("[Meritto MHA] Trigger button clicked!");
            try {
              const widget = window['npfW' + btnId];
              if (widget && typeof widget.showPopup === 'function') {
                console.log("[Meritto MHA] Calling showPopup via widget instance");
                widget.showPopup(btnId, baseUrl);
              } else {
                console.warn("[Meritto MHA] showPopup function not found on widget. Falling back to default click behavior.");
              }
            } catch (err) {
              console.error("[Meritto MHA] Error inside trigger button onclick:", err);
            }
          };
        } else {
          console.error("[Meritto MHA] Trigger button not found during onclick binding!");
        }
        return true;
      }
      return false;
    ...
    if (!initWidget()) {
      console.log("[Meritto MHA] NpfWidgetsInit not ready yet, setting interval polling...");
      const interval = setInterval(() => {
        if (initWidget()) {
          console.log("[Meritto MHA] Successfully initialized widget via interval polling. Clearing interval.");
          clearInterval(interval);
        }
      }, 100);
      setTimeout(() => {
        clearInterval(interval);
        console.log("[Meritto MHA] Interval polling stopped after 10 seconds.");
      }, 10000);
    } else {
      console.log("[Meritto MHA] Successfully initialized widget immediately.");
    }

    return () => {
      console.log("[Meritto MHA] Hook unmounting. Cleaning up trigger onclick.");
  ...
  const onDownload = () => {
    console.log("[Meritto MHA] onDownload function invoked, redirecting to onApplyNow");
    onApplyNow();
  };
  ...
  const onApplyNow = () => {
    const btnId = '3958b53341cc9f2a1efd7126074cee70';
    const baseUrl = 'widgets.nopaperforms.com';
    console.log("[Meritto MHA] onApplyNow function invoked");
    try {
      const widget = window['npfW' + btnId];
      console.log("[Meritto MHA] Checked widget instance on window:", widget);
      if (widget && typeof widget.showPopup === 'function') {
        console.log("[Meritto MHA] Invoking widget.showPopup directly");
        widget.showPopup(btnId, baseUrl);
      } else {
        const trigger = document.querySelector('.npfWidget-' + btnId);
        console.log("[Meritto MHA] Widget instance not ready, found trigger button:", trigger);
        if (trigger) {
          console.log("[Meritto MHA] Clicking trigger button");
          trigger.click();
        } else {
          console.warn("[Meritto MHA] No trigger button found in DOM. Falling back to direct popup window.");
      ...
      console.error("[Meritto MHA] Error invoking popup", e);
    }
  };
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 scroll-smooth font-sans pb-20 sm:pb-0">
      ...
      <section className="relative overflow-hidden bg-[#143674] pt-4 sm:pt-5 pb-5 sm:pb-8 border-b border-white/10">
        <div className="absolute inset-0 z-0 pointer-events-none">
          <img src="/images/programs/mha_hero_bg.png" alt="MHA Program Background" className="h-full w-full object-cover opacity-25" />
            ...
            <div className="flex-1 md:col-span-7 space-y-1.5 sm:space-y-3">
              ...
              <div className="space-y-1 sm:space-y-2">
                <h1 className="text-2xl sm:text-3xl lg:text-[2.75rem] font-extrabold tracking-tighter text-white drop-shadow-lg leading-tight sm:leading-[1.1]">
                  ...
                  <span className="text-[2rem] sm:text-4xl lg:text-[3.5rem] font-black block mt-1.5 sm:mt-2 text-transparent bg-clip-text bg-gradient-to-r from-primary via-teal-300 to-white leading-none py-1">MHA Course</span>
                </h1>
